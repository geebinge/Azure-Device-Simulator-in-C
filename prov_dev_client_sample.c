...
int main(void)
{
	...
	char* id_scope = IDScope->valuestring; 


    cJSON *FMC_PayLoad = cJSON_GetObjectItemCaseSensitive(json, "FMC_PayLoad"); 
    if (cJSON_IsString(FMC_PayLoad) && (FMC_PayLoad->valuestring != NULL)) { 
        printf("FMC_PayLoad: %s\n", FMC_PayLoad->valuestring); 
    } 
    ...
    else
    {
		...
		//const char* custom_json_payload = "{\"FMC_Region\":\"weu\",\"FMC_DeviceTpye\":\"BCM2\"}";

		prov_device_result = Prov_Device_Set_Provisioning_Payload(prov_device_handle, FMC_PayLoad->valuestring);
